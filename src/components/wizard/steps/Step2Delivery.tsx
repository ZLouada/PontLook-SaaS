...
export default function Step2Delivery({ data, onNext, onBack, isSubmitting }: Step2Props) {
  ...
  return (
    ...
    >
          ...
          {DELIVERY_MODES.map((mode) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col justify-between rounded-2xl border p-4 sm:p-5 text-start transition-all duration-200 active:scale-[0.98] touch-manipulation focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-950 ${
                  isSelected
                    ? 'border-blue-500 bg-blue-950/40 shadow-md ring-1 ring-blue-500'
                    : 'border-white/10 bg-slate-900/60 backdrop-blur-xl hover:border-white/20 hover:bg-slate-800/60 hover:shadow-sm'
                }`}
              >
                <div>
      ...
      {requiresLocation && (
        <section className="rounded-2xl border border-white/10 bg-slate-900/60 backdrop-blur-xl p-4 sm:p-5">
              ...
              return (
                <button
                  ...
                  className={`rounded-xl px-4 py-2.5 min-h-[44px] inline-flex items-center justify-center text-xs sm:text-sm font-medium transition-all active:scale-95 touch-manipulation ${
                    isSelected
                      ? 'bg-blue-600 text-white shadow-sm ring-2 ring-blue-500 ring-offset-1 ring-offset-slate-900'
                      : 'border border-white/10 bg-slate-800/80 text-slate-300 hover:border-white/20 hover:bg-slate-700'
                  }`}
                >
                  {city}
                </button>
            ...
            return (
              <button
                ...
                className={`flex min-h-[52px] items-center justify-between rounded-xl border p-4 text-start transition-all active:scale-[0.98] touch-manipulation ${
                  isSelected
                    ? 'border-blue-500 bg-blue-950/40 shadow-sm ring-1 ring-blue-500'
                    : 'border-white/10 bg-slate-900/60 backdrop-blur-xl hover:border-white/20 hover:bg-slate-800/60'
                }`}
              >
                <div>
          ...
          {CUSTOMIZATION_OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                ...
                {opt.recommended && (
                  <div className="mt-3">
                    <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/20 border border-amber-500/30 px-2.5 py-0.5 text-[10px] font-bold text-amber-300">
